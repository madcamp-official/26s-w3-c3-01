...
class LocalUiTest(unittest.TestCase):
    ...
    def test_player_name_editor_searches_postgres_player_list(self) -> None:
        ...
        self.assertIn("button.textContent=playerDisplayName(player)", self.html)
        self.assertIn("[player.name,player.shortName]", self.html)
        self.assertIn("input.dataset.playerName=player.name", self.html)
        self.assertIn("lookupNames=inputs.map", self.html)
    ...
    def test_manual_names_are_kept_only_for_the_current_session(self) -> None:
        ...
        self.assertIn("manualPlayerNames=lookupNames", self.html)
    ...
    def test_only_precisely_stopped_probability_is_saved_to_shot_history(self) -> None:
        ...
        self.assertIn("trigger:'stopped_0.5s'", self.html)
    ...
    def test_live_match_probability_uses_automatic_server_result(self) -> None:
        ...
        self.assertIn("실시간 세트 승률 예측", self.html)
        self.assertIn("result.setWinProbabilityA", self.html)
        self.assertNotIn("result.matchWinProbabilityA", self.html)
        self.assertNotIn("if(hasLoadedVideo){try{renderLiveMatch", self.html)
        self.assertIn("lastConfirmedSetProbabilityA", self.html)
        self.assertIn("?'재계산 중'", self.html)
        self.assertIn("if(!retaining)resetLiveSetProbability()", self.html)
        self.assertIn("syncLiveMatchPlayerNames(null);resetLiveSetProbability()", self.html)
