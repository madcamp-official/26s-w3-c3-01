...
| 경로 | 설명 |
|---|---|
| `/` 또는 `/index.html` | CueCast 메인 UI |
| `/extension-preview` | Chrome 확장 사이드패널 미리보기 |
| `/extension/sidepanel.css` | 확장 CSS |
| `/extension/sidepanel.js` | 확장 JavaScript |
| `/assets/logo.png` | 로고 |
| `/assets/home.png` | 파비콘·홈 이미지 |
...
```bash
curl -s \
  "http://127.0.0.1:8765/api/v1/prematch/players?league=PBA&active_only=true"
```

### cURL — 샷 성공률

```bash
curl -s -X POST http://127.0.0.1:8765/api/v1/shot-probability \
...
```bash
curl -s -X POST http://127.0.0.1:8765/api/v1/youtube/live/stop \
  -H "Content-Type: application/json" \
  -d '{}'
```
...
점수판 OCR은 Python 라이브러리만으로 동작하지 않습니다. 서버 프로세스가 다음 명령을 실행할 수 있어야 합니다.
